...
async function main() {
  ...
  expectCondition(
    invalidAttempt.status === 400 || invalidAttempt.status === 403,
    `Invalid assessment request should fail with 400/403, got ${invalidAttempt.status}.`,
  );
  ...
  const report = {
    ...
    checks: {
      generationChecksEligible,
      generationChecksSkipReason,
      firstSuccessStatus: firstAttempt?.status ?? null,
      replayStatus: replayAttempt?.status ?? null,
      invalidStatus: invalidAttempt?.status ?? null,
      crossUserReadStatus: crossUserReadParsed?.status ?? null,
