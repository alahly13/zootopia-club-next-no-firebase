...
async function main() {
  ...
  try {
    ...
    // Best-effort: try to solve the visible reCAPTCHA checkbox in automation.
    // If Google serves an image challenge, this may still require manual solve.
    const recaptchaFrames = page
      .frames()
      .filter((frame) => frame.url().includes("/recaptcha/") || frame.url().includes("google.com/recaptcha"));
    for (const frame of recaptchaFrames) {
      ...
      await checkbox.click({ delay: 40 }).catch(() => undefined);
      await new Promise((resolve) => {
        setTimeout(resolve, 1800);
      });
      break;
