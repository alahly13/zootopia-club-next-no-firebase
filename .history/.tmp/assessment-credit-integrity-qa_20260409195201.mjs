...
async function main() {
  ...
  const candidateUsers = users.filter(
    (user) => user.role === "user" && user.status === "active" && user.profileCompleted === true,
  );

  if (candidateUsers.length < 2) {
    throw new Error("Need at least two active non-admin users with completed profiles for QA.");
  }

  const userA = candidateUsers[0];
  const userB = candidateUsers[1];
