...
async function main() {
  ...
  const user = selectedFirestoreUser?.uid
    ? await auth.getUser(selectedFirestoreUser.uid)
    : await selectVerifiedAdminUser(auth, adminEmails);

  if (!user?.uid) {
    throw new Error("No profile-complete user account found for homepage QA.");
  }
  ...
  for (const scenario of SCENARIOS) {
    for (const viewport of VIEWPORTS) {
      ...
      const metrics = await page.evaluate((expected) => {
        ...
        const textNodes = Array.from(document.querySelectorAll("p,span,h1,h2,h3"));
        ...
        const trustCard = trustLabelNode ? trustLabelNode.parentElement : null;
