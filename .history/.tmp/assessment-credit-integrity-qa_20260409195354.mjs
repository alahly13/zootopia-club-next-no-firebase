...
async function main() {
  ...
  if (completedCandidates.length < 1 || activeNonAdminUsers.length < 2) {
    console.log(
      JSON.stringify(
        {
          activeNonAdminCount: activeNonAdminUsers.length,
          profileCompleteNonAdminCount: completedCandidates.length,
        },
        null,
        2,
      ),
    ...
    throw new Error(
      "Need at least one profile-complete active non-admin user and one additional active non-admin user for QA.",
    );
  }

  const userA = completedCandidates[0];
  ...
  const firstAttempt = await postAssessment(userASessionCookie, generationPayload, idempotencyKey);
  ...
  const generationId = firstAttempt.payload.data.generation?.id;
  ...
  const afterFirstA = await getUserCredits(userASessionCookie);
  const afterFirstB = await getUserCredits(userBSessionCookie);

  const usedDeltaA = getNumericDelta(baselineA.usedCount, afterFirstA.usedCount);
  const remainingDeltaA =
  ...
  const usedDeltaB = getNumericDelta(baselineB.usedCount, afterFirstB.usedCount);
  const remainingDeltaB =
  ...
  const replayAttempt = await postAssessment(userASessionCookie, generationPayload, idempotencyKey);
  ...
  const afterReplayA = await getUserCredits(userASessionCookie);
  expectCondition(
  ...
  const crossUserReadParsed = await parseApiResponse(crossUserRead);
  expectCondition(
  ...
  const invalidAttempt = await postAssessment(userASessionCookie, {}, `credit-qa-${runId}-invalid`);
  expectCondition(invalidAttempt.status === 400, `Invalid assessment request should fail with 400, got ${invalidAttempt.status}.`);

  const afterInvalidA = await getUserCredits(userASessionCookie);
  expectCondition(
