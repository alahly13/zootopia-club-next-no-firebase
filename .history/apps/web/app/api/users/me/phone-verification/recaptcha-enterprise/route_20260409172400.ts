...
type RecaptchaEnterpriseSiteVerifyOutcome =
  | {
      ok: true;
      score: number;
      action: string;
...
async function verifyRecaptchaEnterpriseWithSiteVerify(params: {
  secretKey: string;
  token: string;
  requestIp: string | null;
}): Promise<RecaptchaEnterpriseSiteVerifyOutcome> {
  const formBody = new URLSearchParams();
  formBody.set("secret", params.secretKey);
  formBody.set("response", params.token);

  if (params.requestIp) {
    formBody.set("remoteip", params.requestIp);
  }
  ...
  try {
    ...
    return {
      ok: false,
      code: "RECAPTCHA_ENTERPRISE_VERIFY_RESPONSE_INVALID",
      message: "Security verification returned an invalid response.",
      status: 502,
    };
  }
...
export async function POST(request: Request) {
  ...
  if (assessmentConfig) {
    /* CreateAssessment is the preferred enterprise verification path for the
       Settings send-OTP precheck. Keep this server-owned so score/action checks
  ...
  /* Keep siteverify as a temporary compatibility fallback while environments
     migrate to CreateAssessment credentials/configuration. */
  const siteVerifyOutcome = await verifyRecaptchaEnterpriseWithSiteVerify({
    secretKey,
    token,
    requestIp,
  });

  if (!siteVerifyOutcome.ok) {
    return apiError(
      siteVerifyOutcome.code,
      siteVerifyOutcome.message,
      siteVerifyOutcome.status,
    );
  }

  if (siteVerifyOutcome.score < minScore) {
    return apiError(
      "RECAPTCHA_ENTERPRISE_RISK_TOO_HIGH",
      "Security verification blocked this request. Please try again later.",
      403,
  ...
  return apiSuccess({
    score: siteVerifyOutcome.score,
    action: siteVerifyOutcome.action,
    hostname: siteVerifyOutcome.hostname,
  });
}
